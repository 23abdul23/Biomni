...
Use `advanced_web_search_model` from `biomni.tool.literature` to find validated sgRNAs from literature and databases:

```python
from biomni.tool.literature import advanced_web_search_model

# Example usage
results = advanced_web_search_model("sgRNA TP53 validated H. sapiens experimental")
```
